class Expense < ApplicationRecord
  ...
  def description
    "Expense: Filer_id #{filer_id} paid $#{sprintf('%.2f', expense_amount)} to #{name} on #{expense_date.strftime("%m/%d/%Y")}#{expense_description ? " for <em>" + expense_description + "</em>." : ". No expense description provided."}"
  end
end
